...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-[10px] bg-white py-(--card-spacing) text-sm text-card-foreground border border-border shadow-none hover:bg-slate-50/50 transition-colors [--card-spacing:--spacing(4)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(3)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-[10px] *:[img:last-child]:rounded-b-[10px]",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "font-semibold text-[14px] leading-snug group-data-[size=sm]/card:text-[13px]",
        className
      )}
      {...props}
    />
